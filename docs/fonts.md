...
- `<s1> <s2>` - сколько пустых пикселей оставлять справа и слева при печати (для пропорциональной печати).

## Генератор шрифтов

Для создания шрифтов можно использовать скрипт `util/q3font.py`. Этот скрипт требует ситуативных доработок и не дает окончательного результата, однако существенно облегчает создание новых шрифтов.
