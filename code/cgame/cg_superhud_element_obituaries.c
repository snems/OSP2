...
static void CG_SHUDElementObituariesInitializeRuntime(shudElementObituaries_t* element, superhudObituariesEntry_t* entry)
{
	...
	if (entry->attacker == ENTITYNUM_WORLD)
	{
		...
		entry->runtime.attackerVisibleChars = 5;
	}
	else if (entry->attacker >= 0 && entry->attacker < MAX_CLIENTS)
	{
		entry->runtime.attackerVisibleChars = CG_TruncateStringWithCodes(cgs.clientinfo[entry->attacker].name, entry->runtime.truncatedAttacker, entry->runtime.maxVisibleChars);
	}
	else
	{
		entry->runtime.attackerVisibleChars = 0;
	}


	if (entry->target >= 0 && entry->target < MAX_CLIENTS)
	{
		entry->runtime.targetVisibleChars = CG_TruncateStringWithCodes(cgs.clientinfo[entry->target].name, entry->runtime.truncatedTarget, entry->runtime.maxVisibleChars);
	}
	else
	{
		entry->runtime.targetVisibleChars = 0;
	}

	entry->runtime.attackerWidth = element->config.fontsize.value[0] * entry->runtime.attackerVisibleChars;
	entry->runtime.targetWidth = element->config.fontsize.value[0] * entry->runtime.targetVisibleChars;
