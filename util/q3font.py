...
file_in = "/hdd/downloads/enter-sansman/entsans.ttf"
file_out = "/home/snems/src/q3/osp2/assets/gfx/2d/sansman_%s.tga"
...
def get_char_width(x, y):
    ...
    for px in range (0, int(char_size_x)):
        sum = 0
        for py in range (0, int(char_size_y)):
            rgba=image.getpixel((x+px, y+py))
            sum = sum + rgba[3]
            if (sum > 0):
                break;
        if (start == -1):
...
print("img \"gfx/2d/sansman_64.tga\" 48")
print("img \"gfx/2d/sansman_32.tga\" 22")
print("img \"gfx/2d/sansman_16.tga\" 0")
...
for char in q3_ascii_table:
    if char != ' ':
        ...
        draw.text((x+x_offset, y+y_offset), char, (255, 255, 255, 255), font=font, anchor="la")
        begin, width = get_char_width(x, y)

        print(i, int(x), int(y), int(begin), int(width), q3_config_template_sizes[i][5], q3_config_template_sizes[i][6], "//", char) 
    else:
        print(i, int(x), int(y), q3_config_template_sizes[i][3], q3_config_template_sizes[i][4], q3_config_template_sizes[i][5], q3_config_template_sizes[i][6])
    i = i + 1
    x = x + char_size_x
